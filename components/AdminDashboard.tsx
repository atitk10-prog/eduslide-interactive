...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ sessions, onDeleteSession }) => {
    ...
    const [showHistory, setShowHistory] = useState(false); // Default: Hide inactive sessions (History)
                ...
                <>
                    <div className="flex flex-col md:flex-row gap-6 items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            <label className="text-sm font-bold text-slate-500 uppercase tracking-widest cursor-pointer">
                                <input
                                    type="checkbox"
                                    checked={showHistory}
                                ...
                                />
                                Hiện lịch sử (Bài đã đóng)
                            </label>
                        </div>
                    </div>

                    <div className="grid grid-cols-1 gap-6">
                        {filteredSessions.filter(s => showHistory || s.isActive).map((session) => (
